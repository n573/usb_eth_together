       ...
       mode_msg = msg_dual;
        ...
        case SOCK_ESTABLISHED :
            ...
            if(received_size > 0){
                ...
                if(storage_buffer == NULL && received_size > 0) {
                    // strncpy(storage_buffer, buf, received_size); // didnt work
                    /* datasize_t i = received_size;
                    while(i > 0) {
                        storage_buffer[i] = buf[i];
                        i--;
                    } */
                //    strncpy(storage_buffer, buf, sizeof(buf)-1);
                //    printf("storage_buffer: %s\n", storage_buffer);
                //    process_commands(buf); // works
                    copy_to_store(buf);
